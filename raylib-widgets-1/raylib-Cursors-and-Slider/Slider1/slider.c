...
SliderWidget InitSliderWidget(Rectangle bounds, int minValue, int maxValue) {
    ...
    slider.leftHandle = InitSliderHandle(bounds.x, handleStartY, DEFAULT_SLIDER_WIDTH, DEFAULT_SLIDER_HEIGHT, RED, (float)bounds.x, (float)bounds.x + bounds.width);
    slider.rightHandle = InitSliderHandle(bounds.x + bounds.width, handleStartY, DEFAULT_SLIDER_WIDTH, DEFAULT_SLIDER_HEIGHT, BLUE, (float)bounds.x, (float)bounds.x + bounds.width);
...
bool IsMouseOverSliderHandle(Vector2 mousePos, SliderHandle handle) {
    return (mousePos.x > handle.x - handle.width && mousePos.x < handle.x + handle.width &&
...
void HandleSliderWidgetMouseInput(SliderWidget *slider, Vector2 mousePos, bool mouseButtonPressed, bool mouseButtonDown, bool mouseButtonReleased) {
    ...
    if (activeHandle != NULL && activeHandle->isDragging && mouseButtonDown) {
        ...
        // Забезпечуємо, щоб ліва ручка не переходила праву і навпаки
        if (slider->leftHandle.x > slider->rightHandle.x) {
            if (activeHandle == &slider->leftHandle) {
                slider->leftHandle.x = slider->rightHandle.x;
            } else if (activeHandle == &slider->rightHandle) {
                slider->rightHandle.x = slider->leftHandle.x;
            }
