...
// Використовується для вибору контрастного кольору тексту
...
// bounds - позиція і розмір слайдера
...
float Gui_SliderEx(int sliderId, Rectangle bounds, PSF_Font font, const char *textTop, const char *textRight,
    ...
    // --- МАЛЮЄМО ПІДКАЗКУ textTop (підтримка багаторядковості, показ при фокусі миші над слайдером) ---

    if (CheckCollisionPointRec(mousePos, bounds) && textTop && textTop[0] != '\0') {
        int padding = 6;  // Відступи всередині рамки

        // Розбиваємо textTop на рядки по символу '\n' (максимум 10 рядків)
        const char* lines[10];
        int lineCount = 0;

        // Копіюємо текст у тимчасовий буфер, бо strtok модифікує рядок
        char tempText[256];
        ...
        // Знаходимо максимальну ширину серед рядків (рахунок символів UTF-8)
        float maxWidth = 0;
        ...
        // Позиція і розмір прямокутника для підказки над слайдером
        Rectangle tooltipRect = {
            bounds.x + bounds.width / 2.0f - (maxWidth + 2 * padding) / 2.0f,
            bounds.y - totalHeight - 8, // 8 пікселів відступ зверху
            maxWidth + 2 * padding,
            totalHeight
        };

        // Малюємо фон підказки та рамку
        DrawRectangleRec(tooltipRect, Fade(borderColor, 0.9f));
        DrawRectangleLinesEx(tooltipRect, 1, textColor);

        // Виводимо кожен рядок тексту з вертикальним відступом
        for (int i = 0; i < lineCount; i++) {
            DrawPSFText(font,
    ...
    // --- МАЛЮЄМО ПІДКАЗКУ textRight (підтримка багаторядковості, показ при фокусі миші над слайдером) ---

    if (CheckCollisionPointRec(mousePos, bounds) && textRight && textRight[0] != '\0') {
        int padding = 6;  // Відступи всередині рамки

        // Розбиваємо textRight на рядки по символу '\n' (максимум 10 рядків)
        const char* lines[10];
        int lineCount = 0;
        ...
        float lineHeight = (float)font.height;
        // Загальна висота всього текстового блоку з урахуванням міжрядкових інтервалів
        float totalHeight = lineCount * lineHeight + (lineCount - 1) * LineSpacing + 2 * padding;

        // Позиція прямокутника для textRight, центрованого вертикально
        ...
        // Малюємо кожен рядок тексту окремо
        for (int i = 0; i < lineCount; i++) {
            DrawPSFText(font,
    ...
    // --- МАЛЮЄМО ЧИСЛОВЕ ЗНАЧЕННЯ слайдера в прямокутнику справа, нижче textRight ---
