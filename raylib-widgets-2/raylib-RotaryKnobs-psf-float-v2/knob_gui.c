...
extern int fontSize;       // Розмір шрифту для тексту
extern int LineSpacing;    // Міжрядковий інтервал (0 - стандартний)
extern Font font;          // Об'єкт шрифту
...
static void draw_knob(int x_pos, int y_pos, float radius, float knobAngle, float knobValue,
    ...
    // Кути шкали: від -135° до +135° (270° всього)
    // Функція DrawRing малює дугу між startAngle і endAngle у градусах

    // Приклад зон (відносні значення від 0 до 1)
    struct Zone {
        float startNorm;  // початок зони (0..1)
        float endNorm;    // кінець зони (0..1)
        Color color;      // колір зони
    } zones[] = {
        { 0.00f, 0.33f, GREEN },  // зелена зона: 0% - 33%
        { 0.33f, 0.66f, YELLOW }, // жовта зона: 33% - 66%
        { 0.66f, 1.00f, RED }     // червона зона: 66% - 100%
    };

    float rotationOffset = -90.0f; // Зсув на 90 градусів проти годинникової стрілки
    ...
    for (int i = 0; i < sizeof(zones)/sizeof(zones[0]); i++) {
        ...
        DrawRing(center,
                 radius - 12,      // внутрішній радіус
                 radius - 5,       // зовнішній радіус
                 startAngle,
                 endAngle,
                 64,               // кількість сегментів для гладкості
                 Fade(zones[i].color, 0.50f)); // Непрозорість 50%
    }
    ...
    for (int i = 0; i <= 100; i += 10) {
        ...
        float textRadius = innerRadius + 12;
    ...
    // Відображення поточного значення регулятора під ним
    char bufValue[32];
    ...
    // DrawPSFText(font18, x_pos - 15, y_pos + radius + 40, bufValue, 1, colorText);
    DrawPSFText(font18, x_pos - lineWidth/2, y_pos + radius , bufValue, 1, colorText);
}
