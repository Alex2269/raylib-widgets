...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            Gui_Knob_Channel(i, (int)positions[i].x, (int)positions[i].y,
                             labels[i],  TextFormat("%0.2f", values[i]),
                             radius, &values[i], 0.0f, 1.0f, true, BLUE);

            // Виводимо текст із поточним значенням регулятора поруч
            DrawTextEx(font, TextFormat("%s value changed: %.1f", labels[i], values[i]),
