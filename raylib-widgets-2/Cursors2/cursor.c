...
void HandleAllCursorsMouseInput(Cursor *cursors, int count, Vector2 mousePos, bool mouseButtonPressed, bool mouseButtonDown, bool mouseButtonReleased)
{
    for (int i = 0; i < count; ++i)
    {
        ...
        if (count > 1)
        {
            // Для прикладу беремо "інший" курсор як наступний по колу
            otherCursor = &cursors[(i + 1) % count];
            if (otherCursor == &cursors[i]) otherCursor = NULL; // якщо один курсор, то NULL
        }

        HandleCursorMouseInput(&cursors[i], mousePos, mouseButtonPressed, mouseButtonDown, mouseButtonReleased, otherCursor);
    }
}
